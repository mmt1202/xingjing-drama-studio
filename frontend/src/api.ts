...
class API {
  ...
  static async request<T = unknown>(
    ...
    let response = await fetch(url, withAuth(endpoint, { ...defaultOptions, ...options }));
    if (response.status === 401 && await refreshAccessToken()) {
      response = await fetch(url, withAuth(endpoint, { ...defaultOptions, ...options }));
    }
  ...
  static async createApiKey(
    ...
    return this.request("/api-keys", {
      ...
      body: JSON.stringify({ name, expires_days: expiresDays ?? null, scopes }),
    });
  }

  static async createAccountStepUp(password: string, purpose: string): Promise<{ token: string; expiresAt: string }> {
    const envelope = await this.request<{ data: { token: string; expiresAt: string } }>("/account/step-up", {
      method: "POST",
  ...
  /** 删除（吊销）指定 API Key。 */
  static async deleteApiKey(keyId: number, expectedVersion: number, stepUpToken: string): Promise<void> {
    return this.request(`/api-keys/${keyId}?expected_version=${expectedVersion}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json", "X-Step-Up-Token": stepUpToken },
    });
  }
  ...
  static async rotateApiKey(
    ...
    return this.request(`/api-keys/${keyId}/rotate`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Step-Up-Token": stepUpToken },
      body: JSON.stringify({ expected_version: expectedVersion, expires_days: 30, scopes }),
    });
  }
