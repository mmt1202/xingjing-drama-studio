...
import { CommercialApiError, createCommercialClient, type CommercialOrder, type PublishOrderInput, type VersionedCommercialOrder } from "@/xingjing_commercial_client";

type CommercialPageId = "AD-005" | "AD-006" | "AD-007" | "AD-008" | "CR-012" | "CR-013" | "CR-014" | "CR-015" | "CR-016";
type CommandKind = "quote" | "acceptQuote" | "contract" | "delivery" | "accept" | "return" | "dispute" | "resolve" | "freeze" | "resume" | "pay";
...
export function CommercialWorkspace({ pageId, workspaceId, accessToken }: { pageId: CommercialPageId; workspaceId: string; accessToken?: string }) {
  ...
  async function execute(kind: CommandKind, values: Record<string, string>) {
    ...
    try {
      const next = kind === "quote" ? await api.submitQuote(orderId, { amount_minor: Number(values.amountMinor), currency: values.currency, proposal: values.proposal, valid_until: new Date(values.validUntil).toISOString() }, command)
        : kind === "acceptQuote" ? await api.acceptQuote(orderId, values.quoteId, command)
  ...
  async function publish(input: PublishOrderInput) {
    if (!api) return;
    setBusy(true); setError(null);
    try {
      const created = await api.publishOrder(input, { idempotencyKey: identifier(), requestId: identifier() });
      setSelected(created);
      await load(created.order.id);
    } catch (reason) { setError(errorText(reason)); }
    finally { setBusy(false); }
  }

  const visibleError = error ?? (!api ? "请先登录后再访问商单。" : null);
  return <main className="min-h-screen bg-[#0b1018] px-4 py-8 text-slate-100 sm:px-7 lg:px-10"><div className="mx-auto max-w-7xl"><header className="border-b border-white/10 pb-7"><p className="text-xs font-semibold tracking-[.18em] text-teal-200">星镜剧创 · {admin ? "平台商单" : "创作者商单"}</p><div className="mt-3 flex flex-wrap items-end justify-between gap-4"><h1 className="font-serif text-3xl tracking-tight text-white sm:text-4xl">{pageTitles[pageId]}</h1><button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-lg border border-white/15 px-3 py-2 text-sm text-slate-200"><RefreshCw size={15} />刷新服务端状态</button></div></header>{visibleError && <ErrorPanel text={visibleError} onRetry={() => void load()} />}{api && loading ? <Loading /> : <div className="mt-7 space-y-6">{pageId === "AD-008" && <PublishOrderCard workspaceId={workspaceId} busy={busy} onPublish={publish} />}<div className="grid gap-6 lg:grid-cols-[18rem_minmax(0,1fr)]"><OrderRail orders={orders} selectedId={selected?.order.id} busy={busy} onPick={(id) => void pick(id)} /><section>{selected ? <OrderSurface pageId={pageId} order={selected.order} busy={busy} onExecute={execute} /> : <EmptyOrders />}</section></div></div>}</div></main>;
}

function PublishOrderCard({ workspaceId, busy, onPublish }: { workspaceId: string; busy: boolean; onPublish: (input: PublishOrderInput) => Promise<void> }) {
  const [title, setTitle] = useState("");
  const [requirements, setRequirements] = useState("");
  const [currency, setCurrency] = useState("CNY");
  const [milestones, setMilestones] = useState([{ title: "", amountMinor: "", acceptanceCriteria: "", dueAt: "" }]);
  const budget = milestones.reduce((sum, item) => sum + Number(item.amountMinor || 0), 0);
  function update(index: number, key: keyof (typeof milestones)[number], value: string) { setMilestones((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, [key]: value } : item)); }
  function submit(event: FormEvent) {
    event.preventDefault();
    if (!title.trim() || !requirements.trim() || milestones.some((item) => !item.title.trim() || Number(item.amountMinor) <= 0 || !item.acceptanceCriteria.trim())) return;
    void onPublish({ owner_workspace_id: workspaceId, title, requirements, budget_minor: budget, currency, milestones: milestones.map((item) => ({ title: item.title, amount_minor: Number(item.amountMinor), acceptance_criteria: item.acceptanceCriteria, due_at: item.dueAt ? new Date(item.dueAt).toISOString() : null })) });
  }
  return <form onSubmit={submit} className="rounded-2xl border border-teal-300/20 bg-teal-300/[.04] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold tracking-[.16em] text-teal-200">发布真实商单</p><h2 className="mt-2 font-serif text-xl text-white">需求、预算与验收节点</h2></div><p className="text-sm text-slate-300">预算合计：{money(budget, currency)}</p></div><div className="mt-4 grid gap-3 md:grid-cols-3"><label className="text-xs text-slate-400">所属工作区<input value={workspaceId} disabled className="mt-1.5 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-500" /></label><label className="text-xs text-slate-400">商单标题<input required value={title} onChange={(e) => setTitle(e.target.value)} className="mt-1.5 w-full rounded-lg border border-white/15 bg-slate-950 px-3 py-2 text-sm" /></label><label className="text-xs text-slate-400">币种<input required value={currency} onChange={(e) => setCurrency(e.target.value.toUpperCase())} className="mt-1.5 w-full rounded-lg border border-white/15 bg-slate-950 px-3 py-2 text-sm" /></label></div><label className="mt-3 block text-xs text-slate-400">制作要求<textarea required value={requirements} onChange={(e) => setRequirements(e.target.value)} className="mt-1.5 min-h-20 w-full rounded-lg border border-white/15 bg-slate-950 px-3 py-2 text-sm" /></label><div className="mt-4 space-y-3">{milestones.map((item, index) => <div key={index} className="grid gap-3 rounded-xl border border-white/10 p-3 md:grid-cols-4"><input aria-label={`里程碑 ${index + 1} 名称`} placeholder="里程碑名称" value={item.title} onChange={(e) => update(index, "title", e.target.value)} className="rounded-lg bg-slate-950 px-3 py-2 text-sm" /><input aria-label={`里程碑 ${index + 1} 金额`} placeholder="金额（分）" type="number" min="1" value={item.amountMinor} onChange={(e) => update(index, "amountMinor", e.target.value)} className="rounded-lg bg-slate-950 px-3 py-2 text-sm" /><input aria-label={`里程碑 ${index + 1} 验收标准`} placeholder="验收标准" value={item.acceptanceCriteria} onChange={(e) => update(index, "acceptanceCriteria", e.target.value)} className="rounded-lg bg-slate-950 px-3 py-2 text-sm" /><input aria-label={`里程碑 ${index + 1} 截止时间`} type="datetime-local" value={item.dueAt} onChange={(e) => update(index, "dueAt", e.target.value)} className="rounded-lg bg-slate-950 px-3 py-2 text-sm" /></div>)}</div><div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={() => setMilestones((items) => [...items, { title: "", amountMinor: "", acceptanceCriteria: "", dueAt: "" }])} className="rounded-lg border border-white/15 px-3 py-2 text-sm">增加里程碑</button>{milestones.length > 1 && <button type="button" onClick={() => setMilestones((items) => items.slice(0, -1))} className="rounded-lg border border-white/15 px-3 py-2 text-sm">删除最后一项</button>}<button disabled={busy} className="ml-auto rounded-lg bg-teal-300 px-5 py-2 text-sm font-bold text-slate-950 disabled:opacity-50">{busy ? "发布中…" : "发布商单"}</button></div></form>;
}
...
function actionsFor(pageId: CommercialPageId): CommandKind[] { if (pageId === "CR-014") return ["quote"]; if (pageId === "AD-005") return ["acceptQuote", "contract"]; if (pageId === "CR-013") return ["delivery"]; if (pageId === "CR-012") return ["accept", "return"]; if (pageId === "CR-016") return ["dispute"]; if (pageId === "AD-006") return ["resolve"]; if (pageId === "AD-007") return ["freeze", "resume", "pay"]; return []; }
...
function CommandCard({ kind, order, busy, onExecute }: { kind: CommandKind; order: CommercialOrder; busy: boolean; onExecute: (kind: CommandKind, values: Record<string, string>) => Promise<void> }) {
  const [values, setValues] = useState<Record<string, string>>(() => ({
    ...
    quoteId: order.quotes.find((item) => item.status === "submitted")?.id ?? "",
