...
vi.mock("@/xingjing_route_bridge", () => ({
  WorkspaceProjectsRoute: ({ workspaceId }: { workspaceId: string }) => <div data-testid="workspace-projects">{workspaceId}</div>,
  ProjectBusinessRoute: ({ workspaceId, projectId, surface, itemId }: { workspaceId: string; projectId: string; surface: string; itemId?: string }) => <div data-testid="project-business">{workspaceId}/{projectId}/{surface}/{itemId}</div>,
  AdminBusinessRoute: ({ workspaceId, surface }: { workspaceId: string; surface: string }) => <div data-testid="admin-business">{workspaceId}/{surface}</div>,
  CollaborationBusinessRoute: ({ workspaceId, projectId, reviewToken, pageId }: { workspaceId?: string; projectId?: string; reviewToken?: string; pageId: string }) => <div data-testid="collaboration-business">{workspaceId}/{projectId}/{reviewToken}/{pageId}</div>,
  WorkspaceAssetsRoute: ({ workspaceId, surface }: { workspaceId: string; surface: string }) => <div data-testid="workspace-assets">{workspaceId}/{surface}</div>,
}));
...
describe("AppRoutes", () => {
  ...
  it("mounts workspace and project business routes with URL context", () => {
    const workspace = renderAt("/app/workspaces/ws-7/projects");
    expect(screen.getByTestId("workspace-projects")).toHaveTextContent("ws-7");
    workspace.unmount();

    renderAt("/app/workspaces/ws-7/projects/project-9/generation");
    expect(screen.getByTestId("project-business")).toHaveTextContent("ws-7/project-9/generation");
  });

  it("mounts workspace-scoped admin routes", () => {
    renderAt("/app/workspaces/ws-7/admin/admin-users");
    expect(screen.getByTestId("admin-business")).toHaveTextContent("ws-7/admin-users");
  });

  it("passes stable asset or shot ids to nested business pages", () => {
    renderAt("/app/workspaces/ws-7/projects/project-9/asset-editor/asset-3");
    expect(screen.getByTestId("project-business")).toHaveTextContent("ws-7/project-9/asset-editor/asset-3");
  });

  it("mounts collaboration pages with workspace and project context", () => {
    renderAt("/app/workspaces/ws-7/projects/project-9/collaboration/CR-014");
    expect(screen.getByTestId("collaboration-business")).toHaveTextContent("ws-7/project-9//CR-014");
  });

  it("mounts customer review pages with token context only", () => {
    renderAt("/review/review-secret/CL-005");
    expect(screen.getByTestId("collaboration-business")).toHaveTextContent("//review-secret/CL-005");
  });
