...
class TaskRepository(BaseRepository):
    ...
    async def find_external_resource_task(
        self,
        *,
        project_name: str,
        task_type: str,
        ...
        """Find a queue task across all states by its external stable ID.

        The active partial unique index alone cannot cover a crash after an
        enqueue commits but before a caller persists the returned legacy task
        ID.  M06 uses this lookup to recover that exact legacy task rather
        than enqueueing a second generation after the first has finished.
        """
        result = await self.session.execute(
            select(Task)
            .where(
            ...
            .order_by(Task.queued_at.desc(), Task.task_id.desc())
            .limit(1)
        )
        row = result.scalar_one_or_none()
        return None if row is None else _task_to_dict(row)
