...
async def _verify_and_get_payload_async(token: str) -> dict:
    ...
    if token.startswith(API_KEY_PREFIX):
        ...
        return payload
    # ArcReel JWT 路径保留兼容；星镜平台访问令牌是数据库支持的随机值，
    # 必须由 Java 身份服务验证，不能在 Python 进程中猜测或复制会话真相。
    payload = verify_token(token)
    if payload is not None:
        return payload
    return await _verify_platform_access_token(token)


async def _verify_platform_access_token(token: str) -> dict:
    import httpx
    ...
    if response.is_error:
        raise HTTPException(status_code=503, detail="身份服务暂时不可用")
    try:
        profile = response.json()["data"]
        user_id = str(profile["id"])
        email = str(profile["email"])
    except (KeyError, TypeError, ValueError) as error:
        raise HTTPException(status_code=503, detail="身份服务响应无效") from error
    return {"sub": email, "user_id": user_id, "via": "xingjing-platform"}
...
def _payload_to_user(payload: dict) -> CurrentUserInfo:
    ...
    return CurrentUserInfo(
        id=str(payload.get("user_id") or DEFAULT_USER_ID),
        sub=sub,
        role="member" if payload.get("via") == "xingjing-platform" else "admin",
    )
