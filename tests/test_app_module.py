...
class TestAppModule:
    def test_marketplace_router_is_mounted_on_main_application(self):
        paths: set[str] = set()
        for included in app_module.app.routes:
            original_router = getattr(included, "original_router", None)
            if original_router is None:
                if path := getattr(included, "path", None):
                    paths.add(path)
                continue
            paths.update(path for route in original_router.routes if (path := getattr(route, "path", None)))
        assert "/api/v1/templates" in paths
        assert "/api/v1/market/items" in paths
