...
def _xingjing_operations() -> set[str]:
    """Operations whose authorization is enforced by the platform identity boundary.

    These handlers validate trusted workspace context, callback credentials, or
    public access tickets themselves. The legacy Bearer-router coverage below
    cannot exercise that boundary when the Java identity service is absent.
    """
    ...
    http_methods = {method.upper() for method in _HTTP_METHODS}
    for included in app.routes:
        ...
        for route in included.original_router.routes:
            ...
            is_platform_handler = (
                endpoint_module.startswith("server.xingjing")
                or endpoint_module == "server.routers.internal_identity_notifications"
                or endpoint_name.startswith("platform_")
            )
            if not is_platform_handler:
                continue
            operation_path = re.sub(r"\{([^}:]+):[^}]+\}", r"{\1}", f"{prefix}{route.path}")
            operations.update(f"{method} {operation_path}" for method in route.methods if method in http_methods)
    return operations
...
def test_every_protected_endpoint_rejects_anonymous(client):
    ...
    platform_operations = _xingjing_operations()
    assert len(operations) > 100, "枚举结果异常，覆盖断言会变成空跑"

    unprotected = []
    for op in operations:
        if op in EXEMPT_OPERATIONS or op in platform_operations:
            continue
