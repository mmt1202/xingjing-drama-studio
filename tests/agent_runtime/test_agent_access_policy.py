...
def _symlink_or_skip(link: Path, target: Path) -> None:
    try:
        ...
        if sys.platform == "win32" and getattr(exc, "winerror", None) == 1314:
            pytest.skip("Windows 当前未授予创建符号链接权限")
        raise
...
def test_pure_construction_with_fake_roots(tmp_path: Path) -> None:
    """假根路径（磁盘上不存在）+ sandbox_enabled 即可纯构造，裁决照常工作。"""
    fake = tmp_path / "nonexistent" / "fake-root"
...
def test_write_protected_with_symlinked_project_cwd_denied(
    ...
    _symlink_or_skip(link_cwd, real_root / "projects" / "selfproj")
